...
// Declare the motor driver instance (Ensure it is initialized in your main code)
extern DriverL298 motorDriver;
...
void PIDControl::updatePID() {
  ...
  // Apply the computed PID output to the motors
  motorDriver.setMotorSpeeds(leftPID.output, rightPID.output);
}
...
double PIDControl::getSpeedRevPerMinute(int i) {
  double encPrevInput;
  encPrevInput = (i > 0 ? rightPID.PrevInput : leftPID.PrevInput);
  return (encPrevInput / (ENCODER_RESOLUTION * PID_INTERVAL) * 60);
}
